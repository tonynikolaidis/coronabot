...
async def on_reactions(payload):
    ...
    if user_id != bot.user.id:
        ...
        # print(user_id, message_id, message_authors)
        ...
        try:
            ...
            # -----------------------           LINEAR           -----------------------------
                ...
                embed = discord.Embed(
                    # title=f"COVID-19 statistics | {data_list[0]['CountryName']} ({data_list[0]['CountryCode']})",
            ...
            # -----------------------           LINEAR - MULTIPLE           -----------------------------
                ...
                if data_list[0]["StatusType"] == "confirmed":
                    embed = discord.Embed(
                        # title=f"Compare COVID-19 statistics (Cases/1M pop.)",
                    ...
                    embed = discord.Embed(
                        # title=f"Compare COVID-19 statistics (Deaths/1M pop.)",
    ...
    #     await ctx.send(embed=embed)
        ...
        if len(countries[0]) == 1:
            if countries[0][0] == 0:
                ...
                embed = discord.Embed(
                    # title=f"{countries[3][0]} ({countries[4][0]})",     # :flag_{countries[4][0].lower()}:
                    description=descriptions[0],
                    timestamp=datetime.utcnow(),
                    colour=discord.Color.blue()
                )
...
async def stats(ctx, *entries):
    # thread = threading.Thread(target=make_stats_embed, args=(ctx, entries))
    # thread.start()
    # print("Spun of thread!")
